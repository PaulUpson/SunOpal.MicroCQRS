...
namespace BigTree.MicroCQRS {
  public class InMemoryEventStore : IEventStore
  {
    ...
    public List<Event> GetEventsForAggregate(Guid aggregateId)
    {
      ...
      return ConvertEventDescriptiors(eventDescriptors);
    }
    ...
    public IDictionary<Guid, List<Event>> GetAllEventsByAggregate() {
      return _current.ToDictionary(item => item.Key, item => ConvertEventDescriptiors(item.Value));
    }
    ...
    private List<Event> ConvertEventDescriptiors(IEnumerable<EventDescriptor> descriptors) {
